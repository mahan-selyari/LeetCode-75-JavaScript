...
### 1. Initial Approach: The "First Timer" Learning Curve
...
### 2. Challenges & Lessons: Syntax & Method Refresher
...
### 3. Final Approach: The "Single Loop" Strategy
...
### 🔍 Further Learning: Is There a Better Way?
...
- **Time Complexity:** `O(N + M)` — Every character from both strings must be processed at least once.
- **Space Complexity:** `O(N + M)` — The result array/string stores all characters from both input strings. This describes the output space; the auxiliary space excluding the returned result is `O(N + M)` for the intermediate result array as implemented.
